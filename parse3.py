...
scales = {
    'features.0': {'in_scale': 0.029044, 'in_zp': 134, 'w_scale': 0.024758, 'w_zp': 0, 'out_scale': 0.088611, 'out_zp': 0},
    'features.4': {'in_scale': 0.088611, 'in_zp': 0, 'w_scale': 0.002032, 'w_zp': 0, 'out_scale': 0.049517, 'out_zp': 0},
    'features.8': {'in_scale': 0.049517, 'in_zp': 0, 'w_scale': 0.003647, 'w_zp': 0, 'out_scale': 0.022357, 'out_zp': 0},
    'classifier.2': {'in_scale': 0.022357, 'in_zp': 0, 'w_scale': 0.007006, 'w_zp': 0, 'out_scale': 0.413417, 'out_zp': 0}
}
...
def conv2d(input, weight, bias, in_scale, in_zp, w_scale, w_zp, out_scale, out_zp):
    ...
    for oc in range(out_channels):
        ...
        acc += bias[oc]  # already in int32 format
...
def dense(input, weight, bias, in_scale, in_zp, w_scale, w_zp, out_scale, out_zp):
    input = input.astype(np.int32) - in_zp
    weight = weight.astype(np.int32) - w_zp
    acc = weight @ input
    acc += bias.astype(np.int32)
    acc_fp = acc.astype(np.float32) * (in_scale * w_scale / out_scale)
    acc_fp += out_zp
